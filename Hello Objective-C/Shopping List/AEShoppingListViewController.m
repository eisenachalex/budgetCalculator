//
//  AEShoppingListViewController.m
//  Shopping List
//
...
@interface AEShoppingListViewController ()
@property (nonatomic) NSArray *items;
@property (nonatomic) NSArray *shoppingList;
@end

@implementation AEShoppingListViewController
...
- (id)initWithStyle:(UITableViewStyle)style {
    ...
    if (self) {
        ...
        [self loadItems];
        // Add Observer
        [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(updateShoppingList:) name:@"AEShoppingListDidChangeNotification" object:nil];
    }
    return self;
}
...
- (void)buildShoppingList {
    ...
    for (int i = 0; i < [self.items count]; i++) {
        AEItem *item = [self.items objectAtIndex:i];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    // Fetch Item
    AEItem *item = [self.shoppingList objectAtIndex:[indexPath row]];
    // Configure Cell
